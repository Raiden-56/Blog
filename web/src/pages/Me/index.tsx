...
const Me = ({ user }: { user: UserInterface | null }) => {
    ...
    return user ? (
        <div>
            <Header/>
            <div className="Me-username" style={{ marginLeft: 'calc(var(--avatar-margin-left) + var(--avatar-width) + 1rem)' }}>@{user.username} <span className="role-admin">#{user.role.name}</span></div>
